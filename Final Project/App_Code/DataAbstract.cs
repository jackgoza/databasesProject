...
namespace Tester
{
    public class DataAbstract
    {
        ...
        public DataSet get_Transactions(long acct_num)
        {
            ...
            string selectSQL = "SELECT TransactionID, Description, Deposit, Name, TransDate, TransType FROM Transactions JOIN Categories ON Transactions.CategoryID = Categories.CategoryID WHERE AcctNumber = " + acct_num + " ORDER BY TransDate DESC";
            SqlCommand cmd = new SqlCommand(selectSQL, connect);
        ...
        public DataSet get_Bound_Transactions(long acct_num, DateTime stDate, DateTime endDate, int category, int sort)
        {
            ...
            {
                string selectSQL = "SELECT * FROM Transactions JOIN Categories ON Transactions.CategoryID = Categories.CategoryID WHERE AcctNumber = " + acct_num + " AND TransDate >= @stDate AND TransDate <= @endDate"; //ordered by date
        ...
        public DataSet get_desc_Transactions(long acct_num, DateTime stDate, DateTime endDate, string desc)
        {
            DataSet temp = new DataSet();
            string selectSQL = "SELECT * FROM Transactions JOIN Categories ON Transactions.CategoryID = Categories.CategoryID WHERE AcctNumber = " + acct_num + " AND TransDate >= @stDate AND TransDate <= @endDate ";//ordered by date
            selectSQL += "AND Description LIKE '%' + @description + '%' " + "ORDER BY TransDate DESC";
